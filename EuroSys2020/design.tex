...
Funk rebalance (on-disk) happens much less frequently. 
Splits  create new chunks as well as new  funks and munks.
...
\sys\ allows arbitrary operations to execute concurrently. Gets are wait-free and proceed without any synchronization. 
In order for scans to be atomic, they synchronize with puts, potentially waiting for some puts to complete
(though puts never wait for scans). Puts also synchronize with rebalance operations (invoked in background threads). 
...
In case of rebalance, the replacement of a chunk, funk, or munk with a new one
%(due to a split) or rebalance of a funk or munk %(see Section~\ref{ssec:rebalance}) %associated with a given chunk 
...
\Cref{alg:ops}  presents pseudocode for the \sys's operations. 
...
\paragraph{Get.}
%\code{Gets} proceed without synchronization. 
...
\paragraph{Put.}
Upon locating the chunk,  a {put} grabs its \code{rebalanceLock} in shared mode to ensure that the chunk is not being rebalanced.
...
Finally, a put unregisters itself from \code{PO}, indicating completion, and releases the chunk's lock.
...
The threshold for funk rebalance is higher than for munk rebalance, causing most rebalances to occur in-memory.
...
Munk rebalance acquires the chunk's \code{rebalanceLock} in shared mode to 
...
%
The algorithm iterates through the \code{PO} to collect the minimum version number among all active scans. 
...
When the new munk is ready, the munk reference in the chunk is flipped and \code{rebalanceLock} is released.
...
When the merge completes, \sys\/ performs catch-up as follows: 
(1) new puts are blocked using the \code{rebalanceLock};
(2) the new log consists of the diverted puts;
...
A split proceeds in two phases. In the first, the chunk is immutable, namely,  \code{rebalanceLock} is held in exclusive mode, but this phase is short. In the second (longer) phase, the new chunks are mutable.
...
In the second phase, puts are enabled on the new chunks (\code{rebalanceLock} is released)
...
%are passed to the OS but might
...
It begins by fetching-and-incrementing GV to obtain a snapshot version \code{gv}.  Next, 
