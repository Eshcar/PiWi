...
Note that this generates a data distribution that is different from the production dataset in~\S\ref{ssec:prod} -- namely, 
all data ranges are populated equally.
} 
...
% Experiments with scans perform 4M to 16M accesses, depending on the scan size. 
...
additionally exercises the Uniform access pattern (\inred{red}). \sys\/ results are depicted with solid
