...
into a table ordered by app id and timestamp. As described in~\S\ref{sec:intro}, the app id distribution 
is heavy-tailed, i.e., the table content is spatially local. 

The experiment consists of two stages - ingestion and analytics. In the first stage, all operations 
are puts (in timestamp order). We ingest 64GB, 128GB and 256GB of data in 12 workers. In the 
second stage, 95\% of the operations are range queries, whereas the rest are puts (lagging data or fixes).  
...
Figure~\ref{fig:prod:ingestion:a} depicts the ingestion rates. \sys's advantage gets more pronounced with 
the dataset's growth. For example, it ingests 256GB of data within 1.1 hours, whereas RocksDB requires 4.8 hours 
(4.4x speedup). Figure~\ref{fig:prod:ingestion:b} depicts the ingestion rate dynamics during the 256GB experiment.
Although RocksDB's throughput is stable overall, it suffers from long periods (1min or longer) of degraded performance 
(10\% of the average rate), due to compactions. \sys's throughput is much noisier, due to frequent munk rebalances; 
however, it prevails even at its slowest. Figure~\ref{fig:prod:ingestion:c} summarizes the write amplification
factors. While \sys's write amplification is hardly affected by the dataset size, RocksDB's deteriorates as the dataset 
(and consequently, the number of LSM levels) grows. For instance, RocksDB's amplification factor for 256G is 11.4x, 
whereas \sys's is only 2.6x. 
