...
\begin{tabular}{lcccc}
\hline 
 & Total CPU time, hr & Read I/O, GB & Write I/O, GB\\
\hline 
\sys & 14.6 & 	47.6 	&  645.2	\\
RocksDB & 10.4 &  2053.5 & 2660.4	\\
\hline 
\end{tabular}
}}}
\caption{{\sys\/ vs RocksDB CPU and I/O utilization under ingestion (100\% put) workload, with a 256GB production dataset.}}
...
\end{subfigure}
\caption{\sys\/ vs RocksDB throughput,  under scan-dominated (95\% scan/5\% put) workload, 
...
(note that it is different from the KV-store primary key). 

Figure~\ref{fig:prod:ingestion:a} depicts the ingestion rates. \sys\/ is much faster; its advantage 
gets more pronounced with the dataset's growth. For example, \sys\/ ingests 256GB of data within 1.1 hours, 
whereas RocksDB requires 4.85 hours (4.5x speedup). Figure~\ref{fig:prod:ingestion:b} depicts the  
throughput dynamics with the 256GB dataset. RocksDB's throughput, while stable overall, suffers from long (1min 
and above) stalls, caused by compactions. 
%\sys's throughput is noisier, because munk rebalances are more frequent than munk compactions; 
\sys, in contrast, delivers predictable performance, and never falls below 4x RocksDB's average rate.
...
experiment. It exhibits a clear dichotomy: (1) \sys\/ is CPU-intensive -- it exploits 40\% more CPU
cycles than RocksDB, i.e., its average CPU rate is 6.3x higher; (2) RocksDB is IO-intensive -- e.g., 
it reads 43.2x (!) more data than \sys\/ (recall that the workload is write-only). Indeed, \sys\/ exploits 
the workload's spatial locality, and spends most of its time handling RAM-resident data; it hardly 
...
it goes through multiple compactions, which degrade its application-level throughput beyond 90\%. 
...
a data distribution that is different from the production dataset in~\S\ref{ssec:prod} -- namely, 
all data ranges are populated equally}). The second phase is warm-up, running read operations 
to warm the caches. The third phase exercises the specific scenario; all worker threads  follow the same access pattern. 
...
We also introduce a new workload, P, comprised of $100\%$ puts (\inred{the data ingestion scenario, 
similar to~\S\ref{ssec:prod}}).
