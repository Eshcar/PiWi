...
We focus on asynchronous persistence -- \inred{i.e., flushes to disk happen in the background}
...
\inred{In all experiments, \sys\/ completes the experiment 20\% to 30\% faster than RocksDB. 
We see that the two systems' executions are very different. \sys's throughput stabilizes 
...
beyond 90\%. After all compactions are done, RocksDB's improved organization gives it an advantage 
(in large datasets) over \sys\/ by up to 23\%, because the latter permanently drains performance in searching
within logs of non-popular funks. We note that this tradeoff between the maintenance cost (for compaction) 
...
Each experiment first  loads a sequence of KV-pairs, \inred{ordered by key,} to an initially empty store, then  
...
\inred{The YCSB workloads are different from the production workloads in~\cref{ssec:prod}, in two 
aspects: (1) the key-ordered dataset produced by YCSB initialization is not spatially-local, and (2) only the 
...
\inred{We did not compare \sys\/ directly against the MySQL default KV-store, InnoDB, because the latter is not easily separable 
from the database code. However, previous evaluations showed that InnoDB is inferior to both RocksDB and TokuDB under 
