...
experiment. It exhibits a clear dichotomy: (1) \sys\/ is CPU-intensive -- e.g., its total and user CPU rates 
exceed RocksDB's by 6.3x and 10x, respectively; (2) RocksDB is IO-intensive -- e.g., its disk read rate 
is 10x \sys's (recall that the workload is write-only). Indeed, \sys\/ exploits the workload's spatial locality, 
and spends most of its time handling RAM-resident data; it hardly ever reads from disk (only upon funk 
rebalances, which are infrequent). RocksDB, on the other hand, spends a large fraction of its time in 
compactions, which are insensitive to locality; hence, it re-reads and re-writes the same data many times. 
