...
Typically, the first attribute %-- called the \emph{primary key} -- 
has a skewed distribution, and so   access via the composite key exhibits \emph{spatial locality}, as 
popular \inred{key prefixes} result in popular key ranges. 
...
\inred{over half an hour of collected data}.  
...
are also the basic units for DRAM \inred{read-write} caching, I/O-batching, logging, and compaction. 
...
\inred{Typical KV-stores rely on OS- and application-level page caches, and employ a global WAL. 
B-trees, e.g., InnoDB, use read-write page caches~\cite{InnoDB-Buffer-Pool}, however they fail to capture spatial locality 
because their cache granularity is too high, which results in poor-performing near-random writes.  
LSM's, e.g., RocksDB, use caches for the read path only. Their temporal structure optimizes write
