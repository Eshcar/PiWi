...
In~\S\S\ref{ssec:layout} we discuss \sys's data organization. 
We move to atomic scans in~\S\S\ref{ssec:scans}, and summarize \sys's
normal (maintenance-free) operation flow in~\S\S\ref{ssec:ops}.  
The data structure's maintenance is discussed in~\S\S\ref{ssec:rebalance}.
Finally,~\S\S\ref{ssec:flush-recovery} discusses data flushes and failure recovery.
...
is acquired in shared mode by puts and in exclusive mode by rebalance, serves this purpose (see~\cref{ssec:layout}).   
...
through a separate exclusive lock (see~\S\S\ref{ssec:layout}). %s\code{funkChangeLock}, 
...
\inred{This guarantees the consistency invariant: at any given moment, all updates pertaining to versions smaller than or equal to the version recorded in the checkpoint file have been persisted.}
