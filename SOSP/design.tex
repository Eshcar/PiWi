...
a read-write lock that is acquired in shared mode by puts, and in exclusive mode by rebalance. 

The algorithm iterates through the \code{PO} to collect the maximum version number among the active scans that cannot be reclaimed yet. 
...
or the ABA number in the entry is increased. When the new munk is ready, the procedure flips the munk reference
and releases \code{rebalanceLock}.

Funk rebalance creates a (SSTable, log) pair. \sys\/ executes this procedure in a dedicated daemon thread. If the chunk has a munk, 
...
(1) new puts are blocked puts through \code{rebalanceLock},
...
\paragraph{Splits and chunk lifecycle.} 
\begin{figure}[tb]
\centerline{
