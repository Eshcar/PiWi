...
First we emphasize the key design differences between LSM (Figure~\ref{fig:lsm}), B$^\epsilon$-trees  (Figure~\ref{fig:bepsilon}), and \sys\ (Figure~\ref{fig:piwi}).

LSM organizes data by temporal partitions, where most recent data is stored in-memory and the rest resides on disk. Data that appears in-memory and have not yet flushed to disk is persisted in a global WAL. This allows for a simple (yet not fast) recovery proceedure, which essentially replays the log to reconstruct the memory component. To allow for atomic scans production LSM stores manage standrd MVCC policy on top of the LSM-tree.  Each write operation is assigned an auto-incremented sequence number. Since each write adds an additional version to the key it writes to, the memory component often overflows--even if the working set is small enough to fit in memory--and the data is flushed to disk. This causes frequent data compactions on disk, which increses write amplification. To facilitate reads a block cache data structure stores the hot data blocks from files and bloom filters are used to avoid accessing most files that do not store the searched key.
 
B$^\epsilon$-tree is a tree with asymptotic worst-case performance guarantees in a model with no memory hierarchy or concurrency. Data is partitioned by key. Writes are buffered through internal nodes and in leaves to be applied in batches, and update leaves in-place. 
Like in B-trees supporting efficient atomic scans in B$^\epsilon$-trees is not trivial. Since updates are buffered in-memory in different paths of the tree before they are persisted in the leaves, supporting consitent recovery in multithreaded workloads is also not trivial.

\sys\ partitions data by key, as done in B$^\epsilon$-trees. However, data is organized as a list of large \emph{chunks}, not a tree, and nodes can be accessed directly (without a tree/list traversal) thanks to an auxiliary volatile index.
\sys\ focuses on leveraging the memory hierarchy effectively for spatially-local workloads. 
We support atomic range scans under concurrency. The burden of increasing the version number is carried by the scans, hence, in the absence of scans write operations effectively update the values in-place. Data that appears in-memory is persisted in per-chunck WALs. The same mechanism that allows supporting atomic scans is utilized for guaranteeing consistent recovery even across these multiple logs. Since the content of chunck is often compacted in-memory disk compactions are infrequent  and write amplification is significantely reduced. To facilitate reads hot chuncks are cached in-memort, in addition hot keys that are not part of a hot range are cached in a row cache. Bloom filters are used to avoid accessing parts of the logs that do not store the searched key.

We proceed with a deep dive into the design details of \sys.
...
At run-time, \sys\ holds in memory a list of chunk metadata objects.
...
In addition, we keep a volatile \emph{chunk index} -- a sorted map from keys to chunks. 
Index updates are lazy, so it only offers best-effort expedited search.
%The detailes of the chunk's metadata structure are deferred to the the supplemental material.
...
%It then 
waits for the completion of all pending puts  that might affect it  -- these are puts of keys in the scanned key range, which either do not have a version yet or have versions lower than the scan time.
...
This mechanism is a simplification of the nonblocking put-scan synchronization described by the KiWi data structure~\cite{kiwi}.
...
the order of puts within a version (following the approach in~\cite{kiwi});  we  enforce 
