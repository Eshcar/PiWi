...
B$^\epsilon$-tree is a tree with asymptotic worst-case performance guarantees in a model with no memory hierarchy or concurrency. Data is partitioned by key. Writes are buffered through internal nodes and in leaves to be applied in batches, and update leaves in-place. Therefore supporting atomic scans in an efficient way is not trivial. Since updates are buffered in-memory in different paths of the tree before they are persisted in the leaves supporting consitent recovery in multithreaded workloads is also not trivial.
