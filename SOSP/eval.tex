...
\inred{In a separate experiment, we follow the RocksDB performance guide~\cite{RocksDBMemoryTuning} to tune its memory resources.  
The results do not affect our conclusions significantly; we discuss them in~\cref{ssec:drill}.}
...
\end{subfigure}
\caption{
{\sys\/ vs RocksDB throughput (Kops), under multiple workloads with Zipf distributions over composite (two-component) and
...
\end{subfigure}
\caption{{\sys\/ 95\% tail latency analysis by serving component, under a mixed get-put workload (YCSB-A).}}
\label{fig:readstat}
\end{figure}
...
Workload YCSB-D (5\% put, 95\% get, Latest distribution, Figure~\ref{fig:throughput:d})
...
The results are similar to YCSB-A (same rates of gets and puts, albeit with dependent timing). With composite keys, \sys\/ overtakes 
RocksDB by 1.2x to 2.4x, however with simple keys, \sys's throughput is 0.7x vs RocksDB's for the 64GB dataset, due to 
relative inefficiency of search in funk-logs. 
}
...
\begin{figure}[th]
\centering
\includegraphics[width=0.49\textwidth]{figs/scalability_line.pdf}
