...
~\S\ref{ssec:drill} analyzes \sys's sensitivity to different configuration settings and application parallelism. 
Finally, a comparison with PebblesDB is presented in ~\S\ref{ssec:pebbles}.
...
over-tuning; \S\ref{ssec:drill} provides insights on parameter choices.
...
due to better exploitation of spatial locality. 
With simple keys, this scenario is 
particularly challenging for \sys\/ because it serves a lot of gets from disk due to 
low spatial locality. The bottleneck is linear search in funk logs, which keep filling 
up due to the high puts rate.
...
% RocksDB does not have this path, hence its gets are 
...
\sys\/ achieves $1.4$x to $3.2$x throughput vs RocksDB under Zipf-composite, and in Zipf-simple, overcomes RocksDB in small datasets. 
\S\ref{ssec:drill} discusses how scan performance on big datasets could be improved by dynamic adaptation of the 
funk log size limit.
...
%Namely, \sys\/ achieves up to $1.6$x and $1.5$x throughput vs RocksDB with composite and simple keys, respectively.   
Workload YCSB-D (5\% put, 95\% get, Latest-simple distribution, Figure~\ref{fig:throughput:d})
has medium spatial locality and thus exhibits a similar trend to workload YCSB-B; however, the gap is smaller.%, \sys\/ achieves up to $1.3$x throughput vs RocksDB.
