...
over RocksDB in some scenarios~\cite{PebblesDB}. We further attemtped to experiment with TokuDB~\cite{TokuDB} -- 
...
\paragraph{Evaluation methodology.} Each experiment consists of three stages. The first stage builds 
...
measure performance only during steady state (skipping the load and warming phases). 
Most experiments  perform 80 million data accesses. Experiments with scans perform 4 to 16 million accesses, depending 
on the scan size. 

To make sure that the results  reproduce, we run 5 experiments for each data point and present the median measurement. 
Note that since experiments are long,
...
(depicted in green), all benchmarks are run with both  the Zipf-simple (orange) 
and Zipf-composite (blue) key distributions. The YCSB-P (put-only) workload 
also exercises the Uniform access pattern (green \inred{todo: change}). \sys\/ results are depicted with solid
lines, and RocksDB with dotted lines. 

We now discuss the results for the different scenarios.
  
{\bf Put-only (data ingestion)} is tested in 
...
The Uniform workload exhibits no locality of any kind. 
 ...
 degrades  with the data size since compactions cover more keys and therefore  engage more files.
...
The put rate is identical to YCSB-A, but all operations perform a get (before the put or on its own). As gets dominate the throughput under such workloads, YCSB-F is slower than YCBC-A.
%The results are similar to YCSB-A (same rates of gets and puts, albeit with dependent timing). 
With composite keys, \sys\/ overtakes 
RocksDB by 1.2x to 2.4x. With simple keys, however, \sys's throughput is 0.7x vs RocksDB's for the 64GB dataset, due to the inefficiency of log searches. 
}

{\bf Mixed put-get} is represented in workloads YCSB-A (50\% put, 50\% get, Figure~\ref{fig:throughput:a}) and 
YCSB-F (100\% get-modify-put, Figure~\ref{fig:throughput:f}). Note that the latter exercises the usual get and put API (i.e., does not provide atomicity). 
In this scenario, \sys\ works well with composite keys, e.g., in YCSB-A it  achieves $1.4$x to $3.5$x the throughput of RocksDB due to better exploitation of spatial locality. 
...
RocksDB's caching is more effective in this scenario, which reduces the disk-access rate of get operations,  resulting in faster gets. 
...
corroborates our analysis. \sys\/ has faster puts and faster or similar get latencies with composite keys
...
We further break down the get latency in  Figure~\ref{fig:readstat}. 
...
and the respective log search latencies are $2.6$ ms vs $4.2$ ms. This happens because in the latter, (1) puts are more dispersed, 
hence the funks are cached less effectively by the OS; and (2) the disk becomes a bottleneck due to the higher I/O rate.
...
\end{subfigure}
\caption{{\sys\/ get latency breakdown by serving component, under a mixed get-put workload (YCSB-A).}}
\label{fig:readstat}
\end{figure}
...
{\bf Get-dominated} 
workloads (YCSB-B--YCSB-D, Figures~\ref{fig:throughput:b}--\ref{fig:throughput:d}) are 
favorable to 
...
For example, in YCSB-C (100\% get, Figure~\ref{fig:throughput:c}), 
...
RocksDB relies mostly on the OS cache to serve these requests and pays the overhead for invoking system calls. 
...
{\bf Scan-dominated} benchmarks YCSB-E (5\% put, 95\% scan, Figures~\ref{fig:throughput:e10}-~\ref{fig:throughput:e1000})
...
funk log size limit to this workload. (The experiments reported herein all use the default configuration).
...
\caption{{\sys\/ vs RocksDB throughput, under the YCSB-D workload.}}
\label{fig:throughput:d}
\end{figure}
}
