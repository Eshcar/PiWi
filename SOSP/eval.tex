...
(funk rebalances in \sys, compactions in RocksDB), which causes write amplification and hampers performance. 
 ...
 \inred{The Uniform workload evenly disperses the keys among \sys's chunks, with very few redundancies. 
 Consequently, the funk logs grow slowly, and funk rebalances are infrequent. The throughput is therefore 
 absolutely insensitive to the dataset size. In contrast, RocksDB performs compactions frequently, with little 
 effect (no redundancies to eliminate); its throughput degrades quickly with the dataset size.}
 
 \inred{Under Zipf-composite, \sys\ benefits from spatial locality whereas RocksDB's write performance 
...
\inred{Note that \sys's recovery time is not impacted by long funk-logs since the KV-store does not replay them
