...
and Zipf-composite (blue). The YCSB-P (put-only) workload 
...
\paragraph{ Put-only (data ingestion)} is tested in 
...
\remove{
\paragraph{ Get-modify-put.} YCSB implements this in-place update scenario through separate calls to get and put (i.e., the semantics are non-atomic). 
...
\paragraph{ Mixed put-get} is represented in workloads YCSB-A (50\% put, 50\% get, Figure~\ref{fig:throughput:a}) and 
...
(0.75x and 0.7x throughput for the 64GB dataset in YCSB-A and YCSB-F, respectively).
...
\paragraph{ Scan-dominated} benchmarks YCSB-E (5\% put, 95\% scan, Figures~\ref{fig:throughput:e10}-~\ref{fig:throughput:e1000})
...
\label{fig:params:bf}
\end{subfigure}
\caption{{\sys\/ throughput sensitivity to configuration parameters, on the 64GB dataset under  YCSB-A (mixed put-get) and 
...
under  YCSB-A and YCSB-E100  with the Zipf-composite key distribution. 
...
and by $1.6$x for YCSB-E by forgoing the system call overhead. This only partly reduces the gap between
...
In all experiments, \sys\ is consistently better than PebblesDB, with its performance improvements ranging from 1.5x with a single thread
on YCSB-P, to 4.5x with eight threads on YCSB-E10.  In all benchmarks, 
...
\hline 
YCSB-P & YCSB-C & YCSB-E10 & YCSB-E100 & YCSB-E1000 \\
\hline 
