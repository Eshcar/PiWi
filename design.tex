...
%		\Statex  \hspace{21mm}		$C.$\code{PPA[th].done} $\vee$ \code{$C$.PPA[th].ver $>$ gv}
		...
		\EndIf
		\State	search \code{key} in \code{funk.kvstore}	
\EndProcedure
\Statex
...
%		\State \code{munk} $\leftarrow$ $C$.\code{munkPtr} \Comment read atomically with \code{gen}
		...
		\If{\code{munk}}
			\State add  \code{$\langle$key, val, gv$\rangle$} to \code{munk}'s linked list
...
A \code{put}  writes the new key-value pair to the munk (if one exists) and to the funk's \code{wbuf}. 
