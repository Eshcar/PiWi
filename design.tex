...
In our experiments, we keep the \code{wbuf} \inred{an order of magnitude smaller than \emph{or} 
around the same size as?} the \code{kvstore}.
...
To this end, each munk-less chunk holds a \emph{Bloom filter} for the corresponding funk's \code{wbuf}; 
...
Using Bloom filters, we eliminate most of the redundant \code{wbuf}  searches.
To reduce the \code{wbuf} search time in case it does happen, 
we further  partition the Bloom filter into a  handful of filters, each summarizing the 
content of part of the \code{wbuf}; this allows us to know not only whether or not to search the 
\code{wbuf}, but also which part of it to search.  
...
\State ptr \code{bloomFilter} \Comment summary of set of keys in \code{wbuf}
...
Algorithm~\ref{alg:ops}  presents a simplified pseudocode for the normal flow of \code{get} and \code{put} operations, without rebalance.
...
\remove{
%% Idit: I think this is actually needed for correctness!!!
...
%		\Statex  \hspace{21mm}		$C.$\code{PPA[th].done} $\vee$ \code{$C$.PPA[th].ver $>$ gv}
		...
		\EndIf
		\State	search \code{key} in \code{funk.kvstore}; return  if found
		\State return NULL	
\EndProcedure
\Statex
...
%		\State \code{munk} $\leftarrow$ $C$.\code{munk} \Comment read atomically with \code{gen}
		\Statex \Comment write  to funk wbuf, munk (if exists), and row cache  
		\State append \code{$\langle$key, val, gv$\rangle$} to \code{funk.wbuf}
		\If{$C$.\code{munk}} 
...
\Procedure{scan}{key1, key2}
		...
		\Repeat
%			\If{$C$.\code{munk}} 
				\State collect from $C$.\code{munk} or $C$.\code{funk} (\code{wbuf} and \code{kstore})
...
A \code{put}  writes the new key-value pair to the  funk's \code{wbuf} and to the 
munk (if one exists). 
...
the \code{kvstore} and the \code{wbuf} and merges the results.
...
The respective funk can be reorganized much less frequently, only in order to bound the \code{wbuf} size. 
