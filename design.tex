...
a compacted and sorted  key-value map \emph{SSTable} and a write buffer \emph{log}. When a funk is created, the \code{SSTable} holds all the chunk's keys with corresponding values, and the \code{log}  is empty.
...
\remove{that holds 10K to 100K keys  ranges between 1M to 100M depending on the data size.} 
...
%\sys\ uses both data structures, and so
So popular key ranges %are associated with munks (and 
...
For working sets that are  larger than the available RAM size, these two caches might not suffice, and so a certain portion of reads are served from disk. Here, the slowest step is the sequential search of the \code{log}. 
...
its associated value. Hence, we would like to reduce \code{log} searches to a minimum. 
