...
\paragraph{Metrics.} Our primary performance metrics are \emph{throughput} and \emph{tail latency}, as produced by YCSB. 
...
\item {\em Latest-simple} -- frequent access to recently added keys. 
 KV-pairs are inserted in sequential key order; the read keys' distribution is skewed towards recently added ones. 
...
database under Zipf-range). As a by-product, \sys\/ is also more 
...
\end{subfigure}
\caption{{\sys\/ {\em get\/} operation statistics by the serving component, under the mixed read-write workload (YCSB-A) and scaling dataset sizes.}}
\label{fig:readstat}
\end{figure}
