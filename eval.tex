...
is 2 MB for write intensive workloads, and $0.5$ MB for other workloads. The Bloom filters for 
munk-less funks are partitioned 16-way.  
...
\hline 
RocksDB, Zipf-range & 1.93	& 2.15 & 2.29 & 2.54	& 2.92\\
\sys, Zipf-range &  1.42	 & 1.38	& 1.27	& 1.30	& 1.29\\
\hline 
...
\sys, syscall  & 0.0 & 0.0	& 0.10 & 0.24 & 0.41\\
\hline 
\end{tabular}
}}
\remove{
...
\hspace{0.05\linewidth}
\begin{subfigure}{0.3\linewidth}
\includegraphics[width=\textwidth]{figs/C_read_amplification_disk_line.pdf}
\caption{Disk I/O}
\label{fig:readamp:d}
\end{subfigure}
\hspace{0.05\linewidth}
\begin{subfigure}{0.3\linewidth}
\includegraphics[width=\textwidth]{figs/C_read_amplification_kernel_line.pdf}
...
\paragraph{Log parameters.} 
We explore the system throughput sensitivity to WAL configuration parameters, for the 64 GB dataset, 
We exercise the YCSB-A and YCSB-E scenarios. Figure~\ref{fig:wal:sz} depicts the dependency on the 
log size limit. Here, too low a threshold (less then 256 KB) causes frequent funk rebalances, which 
destroys the performance. On the other hand, too high a threshold (4 MB) causes longer latencies, 
in both gets and scans. 

Figure~\ref{fig:wal:bf} depicts the dependency on the Bloom filter split factor. 