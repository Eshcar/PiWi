...
as well as the number of read \inred{ and write} system calls. The OS performance counters are retrieved from the Linux
proc filesystem. 
...
In \sys, we allocate 8GB to munks and 4GB to the row cache \inred{unless explicitly indicated otherwise}, 
...
\inred{
The funk log
size limit is 2MB for write intensive workloads, and 512KB for other workloads.
}
...
Figure~\ref{fig:throughput} presents throughput results for the Zipf-simple and 
...
%\end{subfigure}
\caption{{\sys\/ versus RocksDB write amplification under the write-only workload (YCSB-P) and scaling dataset sizes.}}
\label{fig:writeamp}
\end{table}

{\bf Mixed.} YCSB-A (50\% put, 50\% get, Figure~\ref{fig:throughput:a})
...
thanks to better exploitation of spatial locality. But with simple keys, it is faster than RocksDB only for small
working sets that fit in memory, and slower for big datasets.  
...
\inred{under the YCSB-A and YCSB-E scenarios}. 

Table~\ref{fig:wal:sz} depicts the throughput dependency on the log size limit. A low threshold (e.g., 128KB) 
...
workloads.
\inred{
 In other scenarios, funk rebalances are infrequent, hence we set a relatively aggressive 512KB threshold to avoid high tail
latencies. 
}
