...
flushed to disk less frequently than LSM writes (relying on a {\inred {per-chunk WAL's}} for persistence)
yielding 
(3) reduced write amplification, and 
(4) better performance with memory-resident working sets.

{\inred{All \sys\/ APIs (put, get, and scan) provide strong (atomic) consistency guarantees on multiprocessor CPU hardware. 
The system employs fast concurrent data structures for in-memory processing, to scale with the number of cores. 
Finally, \sys\/ provides near-zero failure recovery time since it has no global WAL. }}
