...
It resolves the B-tree write throughput issue through munks and in-memory compaction. \inred{In addition, B-trees heavily rely on exclusive locks for concurrency control~\cite{Lehman81}. \sys\ operations use locks in shared mode in normal flow; exclusive mode is only used by background rebalance operations. }
%which were designed for read-intensive environments, did not focus on strong guarantees in concurrent  scan-write settings. 
...
compactions for random I/O. \sys\/ applies similar ideas to leaf-level storage. \inred{TokuDB's~\cite{TokuDB} performance have been shown to be on par with RocksDB~\cite{tokudb-vs-rocksdb}.}
...
private allocation, and memory-mapped I/O. \sys\/ could incorporate some of these to further improve performance, \inred{however, Tucana does not support consistent recovery.}
