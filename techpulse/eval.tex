...
\remove{It is worth noting that RocksDB's performance has significantly improved over the last two years, primarily through 
optimized LSM compaction algorithms~\cite{CallaghanCompaction}.  
} 
\remove{We also compare against PebblesDB, 
...
We experimented with workload that is derived from Flurry production data to compare performance of \sys\ and RocksDB. 
We also used YCSB~\cite{YCSB}, the  de facto standard  
benchmarking platform for KV-stores to run 
different synthetic workloads.

Our hardware is a 12-core Intel Xeon 5 machine with 4TB SSD disk. 
In order to guarantee fair memory allocation across KV-stores, 
we run each experiment within a Linux container with 16GB RAM. 
%\paragraph{Metrics.} 
Our primary performance metrics are \emph{throughput} 
and \emph{latency percentiles}. 
...
& 64GB & 128GB & 256GB \\
\hline 
RocksDB, throughput &  37,363 &	23,751 & 19,242 \\
\sys, throughput &  89,515 &	90,214 &	71,487 \\
\hline 
RocksDB, write amplification & 5.98	& 9.71	& 11.36	\\ 
\sys, write amplification  & 2.51 & 2.56	& 2.86  \\
\hline 
\end{tabular}
}}
...
Under Flurry highly-skewed workload, \sys\ benefits from spatial locality whereas RocksDB's write performance 
...
& 64GB & 128GB & 256GB \\
\hline 
RocksDB, 1 sec &  5,657 &	4,073 & 2,574 \\
\sys, 1 sec &  10,177 &	7841 &	3,290 \\
\hline 
RocksDB, 10 sec &  3,100 &	2,656 & 2,124 \\
\sys, 10 sec & 4,181 &	3246 &	2,529 \\
\hline 
RocksDB, 1 min & 504	& 381	& 273	\\ 
\sys, 1 min  & 736 & 485	& 349  \\
\hline 
\end{tabular}
}}
...
%It represents workloads with composite keys, such as message threads~\cite{Borthakur:2011:AHG:1989323.1989438},
...
%RocksDB reports a similar benchmark~\cite{rocksdb-benchmarks}; we present it for completeness.
...
%phase exercises the specific scenario; all worker threads follow the same access pattern. 
...
%\centerline{
%\includegraphics[width=0.9\textwidth]{figs/legend0.pdf}
%}
%\end{subfigure}
\caption{
...
%Note that \sys's recovery time is not impacted by long funk logs since they are not replayed upon recovery.
...
\paragraph{ Scan-dominated} benchmark (E1000,  5\% put, 95\% scan, Figure~\ref{fig:throughput:e1000})
